...
public class MockServiceTest {
    ...
    // 1-1 어떤 포지션을 입력해도 미드를 리
    @Test
    public void 어떤포지션을입력해도미드를리턴테스트(){
        Champion champion = mock(Champion.class);
    ...
    public void 이름을야스오로입력하면Exception발생(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void shouldMoreThanTwoTimeRunForChampiongetName(){
        Champion topchamp = mock(Champion.class);
    ...
    @Test
    public void shouldlessThanThreeTimeRunForChampiongetName(){
        Champion topchamp = mock(Champion.class);
    ...
    public void shouldOneTimeRunForChampionGetName(){
        Champion topchamp = mock(Champion.class);
        topchamp.getName();
        verify(topchamp,timeout(200).atLeastOnce()).getName();
    }
    ...
    @Test
    public void 리산드라가짜객체(){
        when(mockService.findByName(anyString())).
                thenReturn(new Champion("리산드라", "미드", 5));
    ...
    public void listSizeTest(){
        ...
        list.add(new Champion("teemo","top",1));
        list.add(new Champion("teemo","top",1));
        when(mockService.findAllChampions()).thenReturn(list);
    ...
    @Test
    public void returnMockAndOnceCalledThenCheckSkin(){
        when(mockService.findByName(anyString())).thenReturn(new Champion("teemo","탑",1));
        assertThat(mockRepository.findByName("애쉬").getHasSkinCount(),is(1));
        verify(mockRepository,times(1)).findByName(anyString());
    }

    // 4. 2개 이상의 가짜 챔피언 객체를 List로 만들어 리턴하고, 하나씩 해당 객체를 검색한 뒤 검색을 위해 호출한 횟수를 검증하세요.
    @Test
    public void 여러개검색호출검층(){
        List<Champion> champions = new ArrayList<>();
        champions.add(new Champion("티모","탑",1));
        champions.add(new Champion("나","탑",1));
        when(mockService.findAllChampions()).thenReturn(champions);
        mockService.findAllChampions().get(0);
        mockService.findAllChampions().get(1);
